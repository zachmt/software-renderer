...
static Arena *get_scratch(Arena *conflict) {
  ...
  Arena *scratch = (conflict == &scratch_arena_global[0])
                       ? &scratch_arena_global[1]
                       : &scratch_arena_global[0];
...
static void free_scratch(Arena *scratch) {
  // By definition the current scratch frame can't be zero because free_scratch
  // must be called after get_scratch
  runtime_assert(scratch->current_scratch_frame != 0);

  u64 size =
      (u64)(((u8 *)scratch->memory_region_start + scratch->bytes_allocated) -
            (u8 *)scratch->current_scratch_frame->frame_base);
...
static void *arena_push(Arena *arena, u64 size, u64 alignment) {
  ...
  u8 *unallocated_start =
      (u8 *)arena->memory_region_start + arena->bytes_allocated;
  ...
  if (arena->bytes_allocated + size > arena->bytes_committed) {
    u8 *uncommitted_start =
        (u8 *)arena->memory_region_start + arena->bytes_committed;
    u8 *page_start =
        (u8 *)align_down_pow2((u64)uncommitted_start, os_get_pagesize());
  ...
  void *res = (u8 *)align_pow2(
      (u64)arena->memory_region_start + arena->bytes_allocated, alignment);
  arena->bytes_allocated += size;
  mem_set(res, 0, size);

  // printf("Arena allocated %lu bytes.\tTotal allocated: %lu bytes.\tTotal
  // committed: %lu bytes.\tTotal reserved: %lu bytes.\n", size,
  // arena->bytes_allocated, arena->bytes_committed, arena->bytes_reserved);
  return res;
}
  ...
  if (start == s.len) {
    ...
    for (u32 i = start + 1; i < s.len; i++) {
      if (!str8_contains(delims, s.data[i])) {
        ...
        if (i < s.len) {
          current->next =
              arena_push(arena, sizeof(Str8Node), align_of(Str8Node));
