...
static Model model_from_obj(Arena *arena, Str8 file_path) {
  ...
  res->texture_vertices =
      arena_push_array(arena, Vec3, res->texture_vertex_count);
  res->vertex_normals =
      arena_push_array(arena, Vec4, res->vertex_normals_count);
  ...
  while (curr) {
    if (str8_equal(curr->str, s("f"))) {
      ...
      res->faces[face_index].flags =
          FACE_NORMALS | FACE_POSITIONS | FACE_TEXCOORDS;
      for (u32 vert = 0; vert < 3; vert++) {
        Str8Node *nums = str8_split(scratch, curr->str, s("/"));
        res->faces[face_index].vertex_indices[vert] =
            (u32)str8_parse_i32(nums->str) - 1;
        nums = nums->next;
        res->faces[face_index].texture_indices[vert] =
            (u32)str8_parse_i32(nums->str) - 1;
        nums = nums->next;
        res->faces[face_index].normal_indices[vert] =
            (u32)str8_parse_i32(nums->str) - 1;
        curr = curr->next;
      }
      ...
      curr = curr->next;
      res->texture_vertices[texture_vertex_index].y =
          str8_parse_f32(curr->str); // TODO: handle case of optional v
      curr = curr->next;
      res->texture_vertices[texture_vertex_index].z =
          str8_parse_f32(curr->str); // TODO: handle case of optional w
      ...
      res->vertex_normals[vertex_normal_index] =
          vec4_norm(res->vertex_normals[vertex_normal_index]);
      vertex_normal_index++;
      curr = curr->next;
    } else if (str8_equal(curr->str, s("v"))) {
      curr = curr->next;
      ...
      // curr = curr->next;
      res->mesh_vertices[mesh_vertex_index].w =
          1.0f; // TODO: handle case of explicit w coordinate
